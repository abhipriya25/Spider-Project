...
class BurgerKingNldDacSpider(scrapy.Spider):
    name = 'burger_king_nl_dac'
    brand_name = 'BURGER KING'
    spider_type = 'chain'
    spider_categories = [Code.FAST_FOOD]
    spider_countries = [pycountry.countries.lookup('nl').alpha_3]
    allowed_domains = ['burgerking.nl']
    start_urls = ['https://www.burgerking.nl/getgeopoints?filter=&restrictToCountry=1&country=nl']

    def parse(self, response):
        '''
        @url https://www.burgerking.nl/getgeopoints?filter=&restrictToCountry=0&country=nl
        '''
