...
class RosselchozbankOfficesSpider(scrapy.Spider):

    name = 'rosselchozbank_offices_dac'
    ...
    def parse_city(self, response):
        ...
        for branchCode in data:
            yield scrapy.FormRequest(
                ...
                method='POST',
                formdata={'branchCode': branchCode.get(),'type': 'offices.list'},
                dont_filter=True,
                callback=self.parse,
    ...
    def parse(self, response, phones, email):
        ...
        for id in list(data):
            ...
            opening_hours = parse_hours(feature['company_mode'])
