...
class BurgerKingNldDacSpider(scrapy.Spider):
    name = 'burger_king_cz_dac'
    brand_name = 'BURGER KING'
    spider_type = 'chain'
    spider_chain_id = '1498'
    spider_categories = [Code.FAST_FOOD]
    spider_countries = [pycountry.countries.lookup('cz').alpha_3]
    allowed_domains = ['www.burgerking.nl']
    start_urls = [
        'https://www.burgerking.nl/getgeopoints?filter=&restrictToCountry=1&country=cz']

    def parse(self, response):
        '''
        @url https://www.burgerking.nl/getgeopoints?filter=&restrictToCountry=0&country=cz
        '''
