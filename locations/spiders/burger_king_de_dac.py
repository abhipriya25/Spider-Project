...
class BurgerKingNldDacSpider(scrapy.Spider):
    name = 'burger_king_de_dac'
    brand_name = 'BURGER KING'
    spider_type = 'chain'
    spider_chain_id = '1498'
    spider_categories = [Code.FAST_FOOD]
    spider_countries = [pycountry.countries.lookup('de').alpha_3]
    allowed_domains = ['www.burgerking.nl']
    start_urls = ['https://www.burgerking.nl/getgeopoints?filter=&restrictToCountry=1&country=de']

    def parse(self, response):
        '''
            @url https://www.burgerking.nl/getgeopoints?filter=&restrictToCountry=0&country=de
        '''
